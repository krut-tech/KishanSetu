...
void main() {
  ...
  group('REAL Runtime Supabase Auth & Profile Integration Verification', () {
    ...
    test('STEP 3A: Login, Role Selection & Profile Setup for FARMER', () async {
      ...
      expect(signInResult.isRight(), isTrue, reason: 'Farmer SignIn should succeed');
      final authResponse = signInResult.getOrElse((_) => throw Exception('Failed'));
      expect(authResponse.user, isNotNull, reason: 'Auth user should be returned');
      ...
      final initialProfile = initialProfileResult.getOrElse((_) => throw Exception('Failed'));
      expect(initialProfile.id, equals(userId));
      expect(initialProfile.fullName, equals('Ramesh Farmer Test'));

      // 3. Select Role
      final roleSelectedProfile = initialProfile.copyWith(role: UserRole.farmer);
      final updateRoleResult = await repository.updateUserProfile(roleSelectedProfile);
      expect(updateRoleResult.isRight(), isTrue);
      final updatedRoleProfile = updateRoleResult.getOrElse((_) => throw Exception('Failed'));
      ...
      final completeProfileResult = await repository.updateUserProfile(completedFarmerProfile);
      expect(completeProfileResult.isRight(), isTrue);
      final savedFarmerProfile = completeProfileResult.getOrElse((_) => throw Exception('Failed'));
    ...
    test('STEP 3B: Login, Role Selection & Profile Setup for BUYER', () async {
      ...
      expect(signInResult.isRight(), isTrue, reason: 'Buyer SignIn should succeed');
      final authResponse = signInResult.getOrElse((_) => throw Exception('Failed'));
      ...
      final completeProfileResult = await repository.updateUserProfile(completedBuyerProfile);
      expect(completeProfileResult.isRight(), isTrue);
      final savedBuyerProfile = completeProfileResult.getOrElse((_) => throw Exception('Failed'));
    ...
    test('STEP 4 & 5: Login, Session Verification, Password Update & SignOut', () async {
      ...
      final profileResult = await repository.getUserProfile(repository.currentUser!.id);
      ...
      final updatePasswordResult = await repository.resetPassword('NewTestPassword123!');
    ...
    test('STEP 6: Send Password Reset Email Request', () async {
      ...
      resetResult.fold(
        (failure) {
          AppLogger.warning('Password Reset Email Request result: ${failure.message}');
          // Accept any valid Failure response from Supabase (rate limit, SMTP config, or failure)
          expect(failure.message, isNotEmpty);
        },
        (_) => AppLogger.info('Password Reset Email Sent Successfully!'),
      );
    });
  });
}
